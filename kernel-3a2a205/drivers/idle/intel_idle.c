		...
		struct cpuidle_driver *drv, int index)
{
	struct cpuidle_state *state = &drv->states[index];
        struct cpuidle_state *c6_state = &drv->states[C6_STATE_IDX];
	...
	if (unlikely(pmu_is_s0ix_in_progress()))
		return intel_idle(dev, drv, C2_STATE_IDX);
	...
	/* Sewell add to disable S0_C6 completely through sysfs
         * c6_state->disable == true if sys/devices/system/cpu/cpu0/cpuidle/state3/disable == 1
         * sleep_state == 0 if C6 is the target state
         */
	if(sleep_state == 0 && c6_state->disable)
		return intel_idle(dev, drv, C2_STATE_IDX);
