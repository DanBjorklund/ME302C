...
static int vibra_pwm_configure(struct vibra_info *info, unsigned int enable)
{
	...
	if (enable) {
		...
		*/
		...
		printk("[vibrator] Enable : set PWM0DUTYCYCLE = 0x63\n");
		...
		printk("[vibrator] Disable : set PWM0DUTYCYCLE = 0x00\n");
	...
	if(info->gpio_pwm > 0){
	   ...
	   if (ret != 0) {
		...
		goto do_freegpio_vibra_enable;
           ...
           ret2 = intel_scu_ipc_ioread8(PWM0CLKDIV1, &data_vib);
           printk("[vibrator] Read PWM0CLKDIV1 = %02X ret = %d\n",data_vib,ret2);
           if (!ret){
		 ret = intel_scu_ipc_iowrite8(PWM0CLKDIV0, 0x4E);
                 ret2 = intel_scu_ipc_ioread8(PWM0CLKDIV0, &data_vib);
	         printk("[vibrator] Read PWM0CLKDIV0 = %02X ret = %d\n",data_vib,ret2);
	   }
	   if (ret){
