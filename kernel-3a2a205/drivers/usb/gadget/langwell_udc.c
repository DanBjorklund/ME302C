...
static int langwell_udc_reset(struct langwell_udc *dev)
{
	...
	devlc = readl(&dev->op_regs->devlc);
	devlc &= ~LPM_ASUS;
	if (build_version == 1 && entry_mode ==1){
		//Eric add for configure usb in full speed mode.  build_version = 1 = eng.
		if (switch_speed == 1)
			devlc |= LPM_PFSC;
		else
			devlc &= ~LPM_PFSC;
...
static ssize_t usb_speed_switch(struct device *dev,
...
static DEVICE_ATTR(speed_switch, S_IRUGO | S_IWUSR | S_IWGRP, 0, usb_speed_switch);
		...
		const struct pci_device_id *id)
{
	...
	//if (build_version == 1)    //disable config usb 1.0 at eng build
		//switch_speed = 1;
...
static int intel_mid_start_peripheral(struct intel_mid_otg_xceiv *iotg)
{
	...
	if (build_version == 1 && switch_speed ==1)
		switch_speed = 2;
