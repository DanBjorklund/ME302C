...
#include "../../power/ME302C_BATTERY/smb347_external_include.h"
...
#if 0
extern int cable_status(int status);
#endif
...
static const char *psc_string(enum power_supply_type charger)
{
	...
	case POWER_SUPPLY_TYPE_USB_ACA:
		return "Accessory Charger Adaptor";
	case POWER_SUPPLY_TYPE_USB_SE1:
		return "Power bank";
				...
				enum usb_charger_type chrg_type)
{
	...
	case CHRG_SE1: return POWER_SUPPLY_TYPE_USB_SE1;
...
int check_cable_status(void)
{
	...
	if (pnw->charging_cap.chrg_type != NULL){
		spin_lock_irqsave(&pnw->charger_lock, flags);
		charger_type = pnw->charging_cap.chrg_type;
		spin_unlock_irqrestore(&pnw->charger_lock, flags);
		if (charger_type == CHRG_SDP)
			return USB_IN;
		if (charger_type == CHRG_DCP  || charger_type == POWER_SUPPLY_TYPE_USB_SE1)
			return AC_IN;
		if (charger_type == CHRG_UNKNOWN)
			return CABLE_OUT;
	}
	else
		return CABLE_OUT;
...
static void penwell_otg_eye_diagram_optimize(void)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		...
		//value = 0x7f;
		value = 0x7d;  //for ASUS ME302C project
	} else {
...
static irqreturn_t otg_id_int(int irq, void *_dev)
{
	struct penwell_otg		*pnw = _dev;

       pr_debug("USB_ID gpio = %d wakeup system\n",pnw->usb0_id_gpio);
	return IRQ_HANDLED;
}
...
static void penwell_otg_work(struct work_struct *work)
{
	...
	case OTG_STATE_B_IDLE:
		if (hsm->id == ID_A || hsm->id == ID_ACA_A) {
			...
			/*Prevent system enter suspend when USB charging type detection*/  
 			wake_lock(&pnw->cable_lock);
			...
			ps_type = pnw->psc_cap.chrg_type;
			spin_unlock_irqrestore(&pnw->charger_lock, flags);
			dev_dbg(pnw->dev,"cable type : %d remove\n",ps_type);
			...
			if (charger_type == CHRG_DCP) {
				...
				penwell_otg_update_chrg_cap(CHRG_UNKNOWN,
						CHRG_CURR_DISCONN);
				penwell_otg_charger_hwdet(false);
			} else if (ps_type == POWER_SUPPLY_TYPE_USB_SE1) {
				/* Notify EM charger remove event */
				penwell_otg_update_chrg_cap(CHRG_UNKNOWN,
						CHRG_CURR_DISCONN);
				penwell_otg_charger_hwdet(false);
...
//2012 Eric :For usb no silent failure 1024
static const char *event_string(enum usb_otg_event event)
{
	...
	//2012 Eric :For usb no silent failure 1024
    	case OTG_EVENT_DVE_DRV_NOT_SUPPORTED:
        	return "NO_DRV_SUPPORTED";
	default:
		return "UNDEFINED";
	}
}

static int penwell_otg_send_event(struct usb_phy *otg,
				enum usb_otg_event event)
{
	...
	dev_info("sending %s event\n", event_string(event));

	snprintf(module_name, 16, "MODULE=%s", driver_name);
	snprintf(udev_event, 128, "EVENT=%s", event_string(event));
	ret = kobject_uevent_env(&otg->dev->kobj, KOBJ_CHANGE, envp);
	if (ret < 0)
		dev_err("uevent sending failed with ret = %d\n", ret);
	return ret;
}
//2012 Eric :For usb no silent failure 1024
		...
		const struct pci_device_id *id)
{
	...
	//2012 Eric:For usb no silent failure 1024
	pnw->iotg.otg.send_event = penwell_otg_send_event;
	//2012 Eric:For usb no silent failure 1024
	...
	wake_lock_init(&pnw->cable_lock, WAKE_LOCK_SUSPEND, "cable_wake_lock");
	...
	//20130326 Eric add for ME302C ++
	pnw->usb0_id_gpio= get_gpio_by_name("USB0_ID");
	retval = gpio_request(pnw->usb0_id_gpio,"USB_ID");
	if (retval < 0) {
		dev_err(pnw->dev, "request usb ID gpio(%d) failed\n",
					pnw->usb0_id_gpio);
		gpio_free(pnw->usb0_id_gpio);
	}else{
		gpio_direction_input(pnw->usb0_id_gpio);
		ret = request_irq(gpio_to_irq(pnw->usb0_id_gpio), otg_id_int,
					IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING,"USB_ID", pnw);
		if (ret)
			dev_err(pnw->dev,"request interrupt %d failed\n", pnw->usb0_id_gpio);
		else
			enable_irq_wake(gpio_to_irq(pnw->usb0_id_gpio));
	}
	//20130326 Eric add for ME302C --
