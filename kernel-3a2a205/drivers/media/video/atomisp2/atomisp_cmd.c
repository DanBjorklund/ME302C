...
//ASUS_BSP+++, RAW patch
static inline void
atomisp_set_sensor_mipi_to_isp(unsigned int pixelformat,
				struct camera_mipi_info *mipi_info)
{
	/* Compatibility for sensors which provide no media bus code
	 * in s_mbus_framefmt() nor support pad formats. */
	if (mipi_info->input_format != -1) {
		/* Whether do RAW capture? */
		if (pixelformat != V4L2_PIX_FMT_NV12) {
			sh_css_input_set_bayer_order(
				mipi_info->raw_capture_bayer_order);
			sh_css_input_set_format(
				mipi_info->raw_capture_input_format);
			v4l2_info(&atomisp_dev,
				"RAW-from-APP: order %d, format %d\n",
				mipi_info->raw_capture_bayer_order,
				mipi_info->raw_capture_input_format);
		} else {
			sh_css_input_set_bayer_order(
				  ...
				  unsigned int source_pad)
{
	...
	int ret;
	unsigned int input_format; //ASUS_BSP+++, RAW patch
	...
	if (isp->inputs[isp->input_curr].type != TEST_PATTERN &&
		...
		//ASUS_BSP+++, RAW patch
		atomisp_set_sensor_mipi_to_isp(pixelformat, mipi_info);

		input_format = mipi_info->input_format;
		if (pixelformat != V4L2_PIX_FMT_NV12)
			input_format = ATOMISP_INPUT_FORMAT_RAW_10;
			  ...
			  unsigned int dvs_env_w, unsigned int dvs_env_h)
{
	...
	format = atomisp_get_format_bridge(pixelformat);
	v4l2_info(&atomisp_dev, "atomisp_set_fmt_to_snr: pixelformat %d, V4L2_PIX_FMT_NV12: %d\n",
			format->pixelformat, V4L2_PIX_FMT_NV12); //ASUS_BSP+++, RAW patch
