...
static int bq27520_proc_bridge_read(char *page, char **start, off_t off, int count, int *eof, void *date)
{
    int len;
...
static int bq27520_proc_read(char *page, char **start, off_t off, int count, int *eof, void *date)
{
	int ret;
...
static int bq27520_thermistor_check(struct i2c_client *client)
{
    ...
    /* ME371MG, ME302C: refer to Gauge IC spec.
...
//ME371MG only
    ...
    if (__fw_cfg_version < 0) {
        ...
        sprintf(__fw_cfg_version, "%s", "xxxx");
    }
    else
