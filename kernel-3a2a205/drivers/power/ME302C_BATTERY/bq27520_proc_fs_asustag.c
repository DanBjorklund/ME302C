...
ssize_t explain_dffs_string(char *OriginalMessages, unsigned long len)
{
    ...
    if (!strncmp(ROM_MODE_WRITE, OriginalMessages, 5)) {
        ...
        if (bq27520_is_rom_mode()) {
            BAT_DBG_E("* Ignore it as recv normal mode command but it's ROM mode *\n");
            return len;
        }
    ...
    /*  check command type, if X need use decimal type to read msleep 
        if command is C or W, slave address(string[3] string[4]) should be 0x16
    */
    ...
    if ('X' == cmd) {
        ...
        regAddrValue=(u8)simple_strtol(p, NULL, 16);
        printk("reg addr string:0X%02X\n", regAddrValue);

        /*  MessageValue[0];I2C slave address will be 16
            MessageValue[1];reg  address
            depend on command type(W/C) do different process.
            do real I2C command(W/C)
        */
        while ((p=strsep(&nextToken, delimit))) {
            pr_debug("%s\n", p);
            MessageValue[MessageCount]=(uint8_t)simple_strtol(p, NULL, 16);
    ...
    /*
	return different error number let parsedffs can do retry
	if('C' == cmd)
		return -EFBIG;
	else if('W' == cmd)
		return len;
