...
// added by cheng_kao for  HWVersion 2012.02.01 ++
...
static IIO_DEV_ATTR_SAMP_FREQ(0660, inv_fifo_rate_show,
	inv_fifo_rate_store);
static DEVICE_ATTR(temperature, S_IRUGO, inv_temperature_show, NULL);
static IIO_DEVICE_ATTR(power_state, 0660, inv_attr_show,
	inv_power_state_store, ATTR_POWER_STATE);
static IIO_DEVICE_ATTR(firmware_loaded, 0660, inv_attr_show,
	inv_attr_store, ATTR_FIRMWARE_LOADED);
static IIO_DEVICE_ATTR(motion_lpa_freq, 0660, inv_attr_show,
	inv_attr_store, ATTR_LPA_FREQ);
static IIO_DEVICE_ATTR(motion_lpa_on, 0660, inv_attr_show,
	inv_attr_store, ATTR_MOTION_ON);
static IIO_DEVICE_ATTR(motion_lpa_duration, 0660, inv_attr_show,
	inv_attr_store, ATTR_MOTION_DURATION);
static IIO_DEVICE_ATTR(motion_lpa_threshold, 0660, inv_attr_show,
...
static IIO_DEVICE_ATTR(compass_matrix, S_IRUGO, inv_attr_show, NULL,
	ATTR_COMPASS_MATRIX);
static IIO_DEVICE_ATTR(dmp_on, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_ON);
static IIO_DEVICE_ATTR(dmp_int_on, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_INT_ON);
static IIO_DEVICE_ATTR(dmp_event_int_on, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_EVENT_INT_ON);
static IIO_DEVICE_ATTR(dmp_output_rate, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_OUTPUT_RATE);
static IIO_DEVICE_ATTR(quaternion_on, 0660, inv_attr_show,
	inv_attr_store, ATTR_DMP_QUATERNION_ON);
static IIO_DEVICE_ATTR(display_orientation_on, 0660,
	inv_attr_show, inv_dmp_attr_store, ATTR_DMP_DISPLAY_ORIENTATION_ON);
static IIO_DEVICE_ATTR(tap_on, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_TAP_ON);
static IIO_DEVICE_ATTR(tap_time, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_TAP_TIME);
static IIO_DEVICE_ATTR(tap_min_count, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_TAP_MIN_COUNT);
static IIO_DEVICE_ATTR(tap_threshold, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_TAP_THRESHOLD);
static IIO_DEVICE_ATTR(pedometer_time, 0660, inv_attr_show,
	inv_dmp_attr_store, ATTR_DMP_PEDOMETER_TIME);
static IIO_DEVICE_ATTR(pedometer_steps, 0660, inv_attr_show,
...
static IIO_DEVICE_ATTR(gyro_enable, 0660, inv_attr_show,
	inv_attr_store, ATTR_GYRO_ENABLE);
static IIO_DEVICE_ATTR(accl_enable, 0660, inv_attr_show,
	inv_attr_store, ATTR_ACCL_ENABLE);
static IIO_DEVICE_ATTR(compass_enable, 0660, inv_attr_show,
	inv_attr_store, ATTR_COMPASS_ENABLE);
#ifdef CONFIG_INV_TESTING
static IIO_DEVICE_ATTR(reg_write, 0660, inv_attr_show,
...
//	SR 1
static struct mpu_platform_data mpu6500_platform_data_SR = {
        .int_config  = 0x10,
        .level_shifter = 0,
        .orientation = {   0,  -1,  0,
                           -1,  0,  0,
...
// SR 2
static struct mpu_platform_data mpu6500_platform_data = {
        .int_config  = 0x10,
        .level_shifter = 0,
        .orientation = {   1,  0,  0,
                           0,  -1,  0,
...
//	TX201LA SR 1
static struct mpu_platform_data mpu6500_platform_data_SR_TX201LA = {
        .int_config  = 0x10,
        .level_shifter = 0,
        .orientation = {   0,  1,  0,
                           1,  0,  0,
	...
	const struct i2c_device_id *id)
{
	...
	HW_SU = Read_HW_ID();
	printk("mpu: mpu6500 HW_SU(%d)\n",HW_SU);
#if CONFIG_ME302C
	if(HW_SU ==  HW_ID_SR1)
	{
		st->plat_data = mpu6500_platform_data_SR;//pdata;
		printk("mpu: HW_ID_SR !!\n");
	}
	else{
		st->plat_data = mpu6500_platform_data;//pdata;
		printk("mpu: HW_ID_SR2 !!\n");
	}
#endif	
	
#if CONFIG_TX201LA
	st->plat_data = mpu6500_platform_data_SR_TX201LA;//pdata;
#endif	
...
{
	struct inv_mpu_iio_s *st = iio_priv(i2c_get_clientdata(to_i2c_client(dev)));
//	pr_debug("%s inv_mpu_resume\n", st->hw->name);
	printk(KERN_ERR "mpu6500 : inv_mpu_resume\n");
...
{
	struct inv_mpu_iio_s *st = iio_priv(i2c_get_clientdata(to_i2c_client(dev)));
	printk(KERN_ERR "mpu6500 : inv_mpu_suspend\n");
