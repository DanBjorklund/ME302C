...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	if (st->chip_config.lpa_mode) {
		...
		if (result)
			goto end_session;
		inv_report_gyro_accl_compass(indio_dev, data,get_time_ns());
		goto end_session;
	}
	...
	if (bytes_per_datum != 0) {
		...
		if( (fifo_count%6)!=0 ){
			printk(KERN_INFO "mpu:fc:%d bpd= %d \n",fifo_count,bytes_per_datum);
			goto flush_fifo;
		}
		...
		if (fifo_count & 1){
			printk(KERN_INFO "mpu6500 : fifo_count odd number\n");
			goto flush_fifo;
		}
		if (fifo_count >  FIFO_THRESHOLD){
			printk(KERN_INFO "mpu6500 : fifo_count  >  FIFO_THRESHOLD\n");
			goto flush_fifo;
		}
		/* timestamp mismatch. */
		if (kfifo_len(&st->timestamps) < fifo_count / bytes_per_datum){
			printk(KERN_INFO "mpu6500 : kfifo_len fail\n");
			goto flush_fifo;
		}
		if (kfifo_len(&st->timestamps) >
			...
			if (st->chip_config.dmp_on) {
				...
				&timestamp, sizeof(timestamp), &copied);
				if (result){
					printk(KERN_INFO "mpu6500 : kfifo_to_user fail\n");
					goto flush_fifo;
				
}
			} else {
				printk(KERN_INFO "mpu6500 : st->chip_config.dmp_on fail\n");
				goto flush_fifo;
			}
		}
	} else {
		result = kfifo_to_user(&st->timestamps,
			&timestamp, sizeof(timestamp), &copied);
		if (result){
			printk(KERN_INFO "mpu6500 : bytes_per_datum = 0 kfifo_to_user fail\n");
			goto flush_fifo;
		}
	}
	tmp = (s8 *)buf;
	while ((bytes_per_datum != 0) && (fifo_count >= bytes_per_datum)) {
		result = inv_i2c_read(st, reg->fifo_r_w, bytes_per_datum,
			data);
		if (result){
			printk(KERN_INFO "mpu6500 : while loop inv_i2c_read fail\n");
			goto flush_fifo;
		}
		result = kfifo_to_user(&st->timestamps,
			&timestamp, sizeof(timestamp), &copied);
		if (result){
			printk(KERN_INFO "mpu6500 : while loop kfifo_to_user fail\n");
			goto flush_fifo;
		}
...
flush_fifo:
	/* Flush HW and SW FIFOs. */
	printk(KERN_INFO "mpu:alp flush_fifo !!!!  \n");
	inv_reset_fifo(indio_dev);
	inv_clear_kfifo(st);
	mutex_unlock(&indio_dev->mlock);
