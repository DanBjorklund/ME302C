...
static int __init sfi_parse_oemr(struct sfi_table_header *table)
{
	...
	PCB_ID = pentry->touch_id | pentry->Camera_1_2M << 2 | pentry->lcd_id << 3 | pentry->hardware_id << 4 | pentry->project_id << 7 | pentry->Camera_3M << 10 | pentry->Camera_1_2M_Lense << 11;
	return 0;
}
...
static int __init sfi_parse_oemb(struct sfi_table_header *table)
{
    ...
    // chipang ++: fix spid temporarily
    oemb->spid.customer_id = 0x0;
    oemb->spid.vendor_id = 0x0;
    oemb->spid.manufacturer_id = 0x0;
    oemb->spid.platform_family_id = 0x2;
    oemb->spid.product_line_id = 0x0;
    oemb->spid.hardware_id = 0xe;
    // chipang --
...
//Joe add for TX201LA switch button driver ++
#ifdef CONFIG_SWITCH_BUTTON
