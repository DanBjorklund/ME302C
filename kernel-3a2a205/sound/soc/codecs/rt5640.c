...
//	{RT5640_ASRC_2		, 0xf800},
	...
	{RT5640_PRIV_INDEX	, 0x006e},// joe_cheng : Add HPF 300Hz by Ryan
	{RT5640_PRIV_DATA	, 0x3259},
	/*playback*/
...
static int checkMclkError(struct snd_soc_codec *codec)
{
	...
	/* backup MX61 value */
	val61 = snd_soc_read(codec,RT5640_PWR_DIG1) & 0x1806;
	/* enable ANC related power */
	ret = snd_soc_update_bits(codec,RT5640_PWR_DIG1,0x1806, 0x1806);
	if (val61 == -1) { // assume i2c error
		try_count++;	
	}
	/* backup MXb8 value */
	valB8 = snd_soc_read(codec,RT5640_ANC_CTRL1);
	ret = snd_soc_write(codec,RT5640_ANC_CTRL1,0x434b);
	if (valB8 == -1) { // assume i2c error
		try_count++;	
	}
	if (try_count !=0 ) {
	        printk("%s : Can't read RT5640 registers, try to reset rt5640\n");
		return 2;
	}
	...
	ret = snd_soc_write(codec,RT5640_ANC_CTRL1,valB8);
	ret = snd_soc_update_bits(codec,RT5640_PWR_DIG1,0x1806, val61);
...
static void mclk_check_work_handler(struct work_struct *work)
{
	...
	int mclk_status;
	mclk_status = checkMclkError(codec);
	...
	if ( mclk_status == 1) {
		...
		i2c_error_count = 0;
	} else if ( mclk_status == 2 ) {
		pr_err("\nRT5640 I2C error, reset codec\n");
		i2c_error_count++;
		if (i2c_error_count >= 10) {
			pr_err("I2C error, Force to reboot the system \n");
			kernel_restart(NULL);
		}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	//ucontrol->value.integer.value[0] = (RT5640_DRC_UPDATE_MASK & snd_soc_read(codec, RT5640_DRC_AGC_1)) ?1:0;
...
/*
       value = (RT5640_DRC_UPDATE_MASK & snd_soc_read(codec, RT5640_DRC_AGC_1)) ?1:0;
	if(ucontrol->value.integer.value[0] == value )
		return 0;
*/
...
static const SOC_ENUM_SINGLE_DECL(rt5640_dmic_enum, 0, 0, rt5640_dmic_mode);
 // Jericho eq
static const char *rt5640_scenario_mode[] = {"Multimedia", "Voip", "RingTone"};
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		msleep(1); // joe_cheng : for HDMI to speaker depop
#ifdef USE_EQ
		if(0==Hal_user_scenario_mode)
			rt5640_update_eqmode(codec,SPK+MULTIMEDIA );
		else
			rt5640_update_eqmode(codec,SPK+VOIP );
#endif
//		if (0x6 > rt5640->v_code)
			mod_timer( &mclk_check_timer, jiffies);
		snd_soc_update_bits(codec, RT5640_PWR_DIG1,
			RT5640_PWR_CLS_D, RT5640_PWR_CLS_D);
		// joe_cheng : remove for no sound. No need to control this register
//		rt5640_index_update_bits(codec,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
#ifdef USE_EQ   // Jericho eq
		if(0==Hal_user_scenario_mode)
			rt5640_update_eqmode(codec,HP+MULTIMEDIA );
		else
			rt5640_update_eqmode(codec,HP+VOIP );
#endif
...
#endif
		break;
	// joe_cheng : workaround for GTalk downlink no sound
	case SND_SOC_DAPM_POST_PMU:
#ifdef USE_ASRC
		snd_soc_write(codec, RT5640_ASRC_1, 0x0000);
		msleep(1);
...
#ifdef USE_ASRC
		// joe_cheng : Fix TT#269900, recording file has noise from 6sec
		// These registers are closed during recording in pre_event. Since post_event and pre_event 
		// are not pair. Move these controls to set_bias_level(). Fixed by Oder.
		//snd_soc_write(codec, RT5640_ASRC_1, 0x9a00);
...
#ifdef USE_ASRC
		// joe_cheng : added by Oder for TT#269900.
		snd_soc_write(codec, RT5640_ASRC_1, 0x9a00);
		snd_soc_write(codec, RT5640_ASRC_2, 0xf800);
#endif
		break;
...
//		}
		if (SND_SOC_BIAS_OFF == codec->dapm.bias_level) {
			...
			snd_soc_update_bits(codec, RT5640_PWR_ANLG1,
				...
				RT5640_PWR_FV1 | RT5640_PWR_FV2);
#ifdef USE_ASRC
			snd_soc_write(codec, RT5640_GEN_CTRL1, 0x3771); // joe_cheng : update to 3771 for power consummation
#else
			snd_soc_write(codec, RT5640_GEN_CTRL1, 0x3701);
#endif
			if (0x5 <= rt5640->v_code) {
				pr_debug("rt5640 version : 0x5\n");
				snd_soc_update_bits(codec, RT5640_JD_CTRL,
					0x3, 0x3);
			}
			if (0x6 <= rt5640->v_code) {
				pr_debug("rt5640 version : 0x6\n");
				snd_soc_update_bits(codec, RT5640_GEN_CTRL1,
					0x0800, 0x0800);
			}
			// joe_cheng
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	...
	// joe_cheng : reset pin, registers reset is not ready.
	gpio = get_gpio_by_name("CDC_RST_PD");
	if (gpio > 0) {
		pr_debug("Already get CDC_RST_PD name\n");
		ret = gpio_request_one(gpio, GPIOF_DIR_OUT, "cdc_rst_pd");
		if (ret) {
			pr_debug("request CDC_RST_PD failed\n");	
		}
		gpio_direction_output(gpio, 1);
		msleep(400);
	} else {
		pr_err("Failed to get CDC_RST_PD\n");
	}
...
static int rt5640_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	// joe_cheng : reset pin
	gpio_direction_output(gpio, 0);
	pr_debug("joe rt5640_suspend CDC_RST_PD %d\n", gpio_get_value(gpio));
	msleep(400);
...
static int rt5640_resume(struct snd_soc_codec *codec)
{
	// joe_cheng : reset pin
	gpio_direction_output(gpio, 1);
	pr_debug("joe rt5640_resume CDC_RST_PD %d\n", gpio_get_value(gpio));
	msleep(400);
